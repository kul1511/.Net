...
namespace CROPDEAL.Repository
{
    public class OrderRepository : IOrders
    {
        ...
        public async Task<bool> AddOrder(OrderDTO newOrder, string userEmail)
        {
            var userId = await _crops.Users.FirstOrDefaultAsync(u => u.Email == userEmail);
            if (userId == null || string.IsNullOrEmpty(userId.Email))
            {
                await LogToDatabase("Failed", $"User Email could not be found to make Order: {userEmail}", DateTime.Now);
            }

            await LogToDatabase("Success", $"Order for User Id: {userId!.UserId}", DateTime.Now);

            newOrder.UserId = userId.UserId;
